...
function initApp() {
    ...
    if (savedState) {
        ...
        currentIndex = savedState.currentIndex || 0; // 【修改】默认选中第1个（顶部），避免无索引时异常
        ...
        fetch('Vocabulary.json')
            ...
            .then(data => {
                words = data;
                toReviewWords = [...words];
                currentIndex = 0; // 【修改】初始化默认选中第1个卡片（顶部）
                updateCounts();
                renderToReviewWords(toReviewWords);
                renderMasteredWords();
                renderUntrainedWords();
    ...
    window.addEventListener('resize', () => {
        ...
        window.resizeTimeout = setTimeout(() => {
            if (isInited) { // 确保应用已初始化
                ...
                debouncedScrollToTarget(); // 【新增】窗口变化后，高亮卡重新置顶
            }
        }, 100);
    });
}
...
// 移至待记忆（右箭头）- 新单词插入顶部，中间列高亮卡置顶
function moveToUntrained() {
    if (toReviewWords.length === 0) return;

    const currentWord = toReviewWords[currentIndex];
    const scrollTop = reviewCardScroll.scrollTop;
    untrainedWords.unshift(currentWord); // 新单词插入顶部
    toReviewWords.splice(currentIndex, 1);
...
function shuffleToReviewWords() {
    ...
    currentIndex = 0; // 【修改】打乱后默认选中第1个（顶部）卡片，置顶显示
    ...
    setTimeout(() => {
        renderToReviewWords(toReviewWords);
        debouncedScrollToTarget(); // 【修改】打乱后高亮卡置顶
        updateCounts();
        saveStateToLocalStorage();
...
function bindEvents() {
    document.addEventListener('keydown', (e) => {
        ...
        switch (e.key) {
            ...
            case 'ArrowUp':
                e.preventDefault();
                switchWord('up'); // 向上切换后置顶
                break;
            case 'ArrowDown':
                e.preventDefault();
                switchWord('down'); // 向下切换后置顶
                break;
            case ' ':
                e.preventDefault();
                switchWord('down'); // 空格切换后置顶
                break;
            case 'Enter':
                e.preventDefault();
                switchWord('down'); // Enter切换后置顶
                break;
            default:
                break;
        }
    ...
    // 【核心修改2】滚动同步选中：基准点改为窗口顶部，距离最近则选中
    reviewCardScroll.addEventListener('scroll', () => {
        ...
        const targetY = containerRect.top; // 窗口顶部坐标
        ...
        cards.forEach((card, index) => {
            ...
            const cardTopY = cardRect.top; // 对比卡片顶部坐标
            const distance = Math.abs(cardTopY - targetY); // 计算卡片顶部到窗口顶部的距离

            // 距离更近则更新；距离相同选下方卡片（避免滚动时选中状态跳跃）
        ...
        if (closestIndex !== currentIndex) {
            ...
            debouncedScrollToTarget(); // 【新增】滚动后自动置顶选中卡片
            updateCounts();
            saveStateToLocalStorage();
        }
    });
