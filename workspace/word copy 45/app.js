...
let words = [];
let currentIndex = 0;
let toReviewWords = []; // 中间列（记忆区）单词数组
...
function renderUntrainedWords() {
    ...
    if (untrainedWords.length === 0) {
        untrainedList.innerHTML = '<div class="empty-state">暂无待巩固单词<br>按→键将中间单词移至此处</div>';
        return;
    }
...
// 切换单词（上下键/空格/Enter）- 中间列（记忆区）切换后置顶
...
// 移至已牢记（左箭头）- 新单词插入左列顶部，中间列（记忆区）高亮卡置顶
...
// 移至待巩固（右箭头）- 新单词插入右列顶部，中间列（记忆区）高亮卡置顶
...
// 绑定核心交互事件（含滚动同步选中逻辑修改，适配中间列=记忆区）
function bindEvents() {
    document.addEventListener('keydown', (e) => {
        ...
        switch (e.key) {
            case 'ArrowLeft':
                e.preventDefault();
                moveToMastered(); // 左箭头：移至左列（已牢记）
                break;
            case 'ArrowRight':
                e.preventDefault();
                moveToUntrained(); // 右箭头：移至右列（待巩固）
                break;
            case 'ArrowUp':
                e.preventDefault();
                switchWord('up'); // 向上切换：中间列（记忆区）单词后置顶
                break;
            case 'ArrowDown':
                e.preventDefault();
                switchWord('down'); // 向下切换：中间列（记忆区）单词后置顶
                break;
            case ' ':
                e.preventDefault();
                switchWord('down'); // 空格：中间列（记忆区）单词切换后置顶
                break;
            case 'Enter':
                e.preventDefault();
                switchWord('down'); // Enter：中间列（记忆区）单词切换后置顶
                break;
            default:
                break;
        }
    ...
    toggleBtn.addEventListener('click', toggleMeaning); // 绑定中间列（记忆区）释义切换按钮事件
    shuffleBtn.addEventListener('click', shuffleToReviewWords); // 绑定中间列（记忆区）随机顺序按钮事件

    // 【核心修改2】滚动同步选中：基准点改为窗口顶部，距离最近则选中（适配中间列=记忆区）
    reviewCardScroll.addEventListener('scroll', () => {
        ...
        // 基准点：窗口顶部（原逻辑是窗口中点）
        const scrollContainer = reviewCardScroll; // 中间列（记忆区）滚动容器
        ...
        if (closestIndex !== currentIndex) {
            ...
            debouncedScrollToTarget(); // 【新增】滚动后自动置顶中间列（记忆区）选中卡片
            updateCounts();
            saveStateToLocalStorage();
        }
    });
