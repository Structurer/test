...
// 绑定核心交互事件（含滚动同步选中逻辑修改，适配新按键功能）
function bindEvents() {
    document.addEventListener('keydown', (e) => {
        // 输入框未聚焦时，处理全局按键（避免重复绑定）
        if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
            ...
            switch (e.key) {
                ...
                case ' ':
                    e.preventDefault();
                    moveToMastered(); // 空格：绑定左箭头功能（移至已牢记）
                    break;
                case 'Enter':
                    e.preventDefault();
                    // 输入框未聚焦时，Enter键不执行操作
