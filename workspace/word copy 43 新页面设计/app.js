...
const debouncedScrollToTarget = debounce(() => {
    ...
    isAutoScroll = true;
    // 计算窗口1/2处（中点）参考点（从下往上也是1/2处）
    const scrollContainer = reviewCardScroll;
    const containerHeight = scrollContainer.clientHeight;
    const containerTop = scrollContainer.getBoundingClientRect().top;
    const targetY = containerTop + containerHeight * 1 / 2; // 窗口中点（核心修改：从2/3改为1/2）

    // 计算卡片顶部的当前Y坐标（比对目标不变，仍为卡片顶部）
    const cardRect = targetCard.getBoundingClientRect();
    const cardTopY = cardRect.top; // 卡片顶部Y坐标

    // 计算需要滚动的距离：让卡片顶部对齐目标Y
    const scrollOffset = scrollContainer.scrollTop + (cardTopY - targetY);
...
function initApp() {
    ...
    // 核心新增1：监听窗口大小变化，强制滚动到顶部（修改：从滚到底部改为顶部）
    window.addEventListener('resize', () => {
        // 窗口缩放时，延迟100ms执行（避免频繁触发）
        clearTimeout(window.resizeTimeout);
        window.resizeTimeout = setTimeout(() => {
            if (isInited) { // 确保应用已初始化
                forceScrollToTop(); // 强制左右列滚到顶部（修改点）
                // 窗口大小变化后，重新对齐选中卡片
...
// 核心修改2：强制左右列滚动到顶部（原forceScrollToBottom修改）
function forceScrollToTop() {
    // 左列强制滚到顶部
    if (masteredList.scrollHeight > 0) {
        masteredList.scrollTop = 0; // 修改：从scrollHeight改为0（顶部）
    }
    // 右列强制滚到顶部
    if (untrainedList.scrollHeight > 0) {
        untrainedList.scrollTop = 0; // 修改：从scrollHeight改为0（顶部）
    }
}
...
function renderMasteredWords() {
    ...
    masteredWords.forEach((wordObj, index) => {
        const isLatest = index === 0; // 修改：最新单词是数组第0个（顶部），原index === masteredWords.length - 1
...
function renderUntrainedWords() {
    ...
    untrainedWords.forEach((wordObj, index) => {
        const isLatest = index === 0; // 修改：最新单词是数组第0个（顶部），原index === untrainedWords.length - 1
...
function moveToMastered() {
    ...
    masteredWords.unshift(currentWord); // 修改：从push改为unshift（插入顶部），原masteredWords.push(currentWord)
...
function moveToUntrained() {
    ...
    const currentWord = toReviewWords[currentIndex];
    const scrollTop = reviewCardScroll.scrollTop;
    untrainedWords.unshift(currentWord); // 修改：从push改为unshift（插入顶部），原untrainedWords.push(currentWord)
    toReviewWords.splice(currentIndex, 1);
...
// 绑定核心交互事件（含长按禁用+滚动选中逻辑修改）
function bindEvents() {
    document.addEventListener('keydown', (e) => {
        if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return;

        // 拦截上下键/空格/Enter的长按连续触发
        ...
        switch (e.key) {
            ...
            case 'ArrowUp':
                e.preventDefault();
                switchWord('up'); // 向上切换，对齐窗口中点
                break;
            case 'ArrowDown':
                e.preventDefault();
                switchWord('down'); // 向下切换，对齐窗口中点
                break;
            case ' ':
                e.preventDefault();
                switchWord('down'); // 空格向下，对齐窗口中点
                break;
            case 'Enter':
                e.preventDefault();
                switchWord('down'); // Enter向下，对齐窗口中点
                break;
            default:
                break;
        }
    ...
    // 中间列滚动同步选中：比对卡片顶部与窗口中点距离，距离相等选下方卡片
    reviewCardScroll.addEventListener('scroll', () => {
        ...
        // 计算窗口中点参考点（从下往上也是1/2处）
        const scrollContainer = reviewCardScroll;
        const containerRect = scrollContainer.getBoundingClientRect();
        const targetY = containerRect.top + containerRect.height * 1 / 2; // 核心修改：从2/3改为1/2
        ...
        cards.forEach((card, index) => {
            ...
            const cardTopY = cardRect.top; // 比对目标：卡片顶部Y坐标
            const distance = Math.abs(cardTopY - targetY); // 计算距离

            // 距离更小时更新选中索引；距离相等时优先选中靠下的卡片（规避跳跃bug）
