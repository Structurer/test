...
function bindSlideToContainer(container, area) {
    ...
    container.addEventListener('mousedown', (e) => {
        ...
        if (area === 'mastered') {
            ...
            // 中间列取点击卡片的单词对象，而非currentIndex对应的
            targetCardObj = toReviewWords.find(item => item.word.trim() === wordText);
            // 同步更新currentIndex为点击卡片的索引，确保滑动后高亮正确
            currentIndex = toReviewWords.findIndex(item => item.word.trim() === wordText);
        }
    });
        ...
        if (Math.abs(slideDistance) >= slideThreshold) {
            switch (currentOperateArea) {
                ...
                case 'review': // 中间列：左滑移左列，右滑移右列
                    ...
                    // 迁移后更新索引，保持高亮在合理位置
                    currentIndex = toReviewWords.length > 0 ? tempIndex : 0;
                    updateAllUI();
                    break;
            }
        }
